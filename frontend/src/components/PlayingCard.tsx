...
import { ACTION_BLURB_KEY, ACTION_ICON, colorMeta } from '../game/meta';
...
function cardTitle(card: Card, t: I18n['t'], tCard: I18n['tCard']): string {
    ...
    if ((card.type === 'property_wildcard' || card.type === 'rent') && cols.length === 2) {
...
export default function PlayingCard({
    card, size = 'md', selected, dimmed, flagged, onClick, className = '', style, title,
    draggable, onDragStart, onDragEnd, dragging,
}: Props) {
    const { t, tCard, tColor } = useI18n();
    const interactive = Boolean(onClick);
    const cols = stripeColors(card);
    const isProp = card.type === 'property' || card.type === 'property_wildcard';
    ...
    return (
        <button
            ...
            className={[
                ...
                selected ? '-translate-y-1.5 shadow-[var(--shadow-glow)]' : '',
        ...
        >
                ...
                <div className="flex h-[26%] w-full border-b border-black/25">
                    {cols.map((c, i) => {
                        ...
                        return (
                            ...
                            >
                                <span className={`truncate px-0.5 uppercase ${cols.length > 1 ? 'text-[0.75em]' : 'text-[0.85em]'}`}>
                                    {cols.length > 1 ? t(`color.short.${c}`) : tColor(c)}
                                </span>
                            </div>
                        );
                    })}
            ...
            {card.type === 'money' && (
                <div
                    ...
                    style={{ background: 'linear-gradient(180deg,#bbf7d0,#4ade80)', color: '#04240f' }}
                >
                    <span className="text-[0.85em] tracking-widest">{t('card.type.bank')}</span>
            ...
            {card.type === 'action' && (
                <div
                    ...
                    style={{ background: 'linear-gradient(180deg,#e9d5ff,#a855f7)', color: '#280046' }}
                >
                    <span className="text-[0.85em] tracking-widest">{t('card.type.action')}</span>
                    <span className="text-[1.1em] leading-none">{card.action ? ACTION_ICON[card.action] : ''}</span>
                </div>
            )}

            <div className="relative flex flex-1 flex-col items-center justify-center gap-0.5 px-1 py-1 text-center">
                {card.type === 'rent' && (
                ...
                {card.type === 'money' ? (
                    <>
                        ...
                        <span className="text-[0.8em] font-bold uppercase tracking-widest opacity-55">{t('card.million')}</span>
                    </>
                ) : (
                ...
                {card.type === 'action' && card.action && size !== 'xs' && (
                    <span className="text-[0.85em] italic opacity-65">{t(ACTION_BLURB_KEY[card.action])}</span>
                )}

                {isProp && size === 'md' && (
                    <span className="text-[0.85em] opacity-55">
                        {t(card.type === 'property_wildcard' ? 'card.type.wildcard' : 'card.type.property')}
                    </span>
                )}
            </div>
            ...
            {card.type !== 'money' && (
                <div className="flex items-center justify-between border-t border-black/15 bg-black/5 px-1.5 py-0.5">
                    <span className="text-[0.8em] font-semibold uppercase opacity-50">{t('card.value')}</span>
                    <span className="font-display text-[1.15em] leading-none">{money(t, card.value)}</span>
                </div>
            )}
        </button>
    );
...
export function CardBack({ size = 'md', className = '', label, style }: {
    size?: Size; className?: string; label?: string; style?: CSSProperties;
}) {
    return (
        <div className={`card-back ${SIZES[size]} grid shrink-0 place-items-center ${className}`} style={style}>
            {label && <span className="font-display text-lg tracking-widest text-white/85">{label}</span>}
        </div>
    );
}
