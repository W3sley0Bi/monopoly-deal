...
function render(seed: string, variant: number): string {
    ...
    switch (variant) {
        case 0:
            return createAvatar(funEmoji, shared).toDataUri();
        case 1:
            return createAvatar(botttsNeutral, shared).toDataUri();
        case 2:
            return createAvatar(adventurerNeutral, shared).toDataUri();
        default:
            return createAvatar(loreleiNeutral, shared).toDataUri();
    }
}

/**
 * A stable data-URI avatar for a player id. Four styles keep a table varied
 * while each person keeps the same face everywhere. Generated locally, so it
 * works with no internet connection.
 */
export function avatarFor(seed: string): string {
    const cached = cache.get(seed);
    if (cached) return cached;
    const uri = render(seed, hash(seed) % 4);
    cache.set(seed, uri);
    return uri;
}
